      ...
      <div className="bg-white rounded-xl shadow-lg border-2 border-black overflow-hidden">
        ...
        <div className="bg-black text-white px-6 py-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-xl font-bold">Extracted Contact Information</h2>
            {isCached && (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {data.email.map((email, index) => (
                      <a
                        key={index}
                        href={`mailto:${email}`}
                        className="inline-flex items-center gap-1 px-3 py-1.5 bg-gray-200 text-gray-900 rounded-full text-sm hover:bg-gray-300 transition-colors"
                      >
                        {email}
                      </a>
                    ))}
                    ...
                    {Object.entries(data.socials).map(([platform, url]) => 
                      url && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="px-4 py-2 bg-gray-200 rounded-lg hover:bg-gray-300 transition-colors text-sm font-medium text-gray-900 capitalize border border-gray-400"
                        >
                          {platform}
                        </a>
                      )
